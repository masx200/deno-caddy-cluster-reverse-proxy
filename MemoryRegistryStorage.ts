...
export function MemoryRegistryStorage(): RegistryStorage {
    ...
    return {
        ...
        async getAddressLastCheck(address: string): Promise<number> {
            return address_to_server_info.get(address)?.last_check_time ?? 0;
        },
        async setAddressLastCheck(
            ...
            if (server_info) {
                server_info.last_check_time = last_check_time;
            }
        },
        ...
        async upsertServerInformation(
            ...
            const { address, name, last_check_time = 0 } = options;
            const set = name_to_address.get(name) ?? new Set();
            set.add(address);
            name_to_address.set(name, set);
            address_to_server_info.set(address, {
                ...options,
                last_check_time,
            });
        },
